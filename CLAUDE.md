...
| 04-26 | VS Code session-join fix | Removed `--unity-launch` (caused Electron to join desktop VS Code instead of rendering in Xvfb). Added `--new-window`, `--disable-dev-shm-usage`, `--disable-breakpad`, `--crash-dumps-dir=/tmp`, `--no-first-run`, `--disable-extensions`, `--window-size`, `--window-position=0,0` |
| 04-26 | VS Code deadlock fix | `Vsc_ShellExec` changed from blocking `wait4(0)` to `WNOHANG` + 50ms retry. Prevents hang when Electron process tree is slow to die |
| 04-26 | VS Code process cleanup | `Vsc_CleanupStale`/`Vsc_StopAll` now use full binary path `/usr/share/code/code` + explicit `chrome_crashpad_handler` kill. Single combined shell command in StopAll |
| 04-26 | Chrome/VSCode pkill isolation | Chrome `CleanupStaleXvfb` now targets `Xvfb :99` instead of bare `Xvfb` — no longer kills VS Code's Xvfb :98 |
| 04-26 | Process isolation fix | **Critical fix:** Replaced all broad `pkill` with PID file system + process group kills (`kill(-pgid, SIGKILL)`). Old approach killed desktop Chrome/VS Code system-wide. PID files written on launch (`/tmp/*_ailang_{xvfb,browser,xdotool}.pid`), read on cleanup for targeted kill. EPERM prints actionable `sudo kill -9 <pid>` message |
| 04-26 | Privilege drop (DropPriv) | `stat("/home/bob")` → `st_uid` (offset 28) + `st_gid` (offset 32) → `setgid(gid)` + `setuid(uid)` before `execve` in all 3 child forks (Xvfb, app, xdotool). Fixes: Electron root refusal, root-owned fbdir/profile, security. Syscalls: SYS_STAT=4, SYS_GETUID=102, SYS_SETUID=105, SYS_SETGID=106 |
| 04-26 | Profile dir cleanup | `CleanupStale` now `rm -rf` entire profile dirs (`/tmp/*_ailang_profile`). Root-owned profiles from prior sessions blocked Chrome/VS Code startup (EACCES on profile write). Apps recreate fresh profile as bob after DropPriv |
| 04-26 | fbdir cleanup | `CleanupStale` now `rmdir` fbdir after unlinking `Xvfb_screen0`. Prevents stale root-owned dirs |
| 04-26 | Resize maximize fix | Chrome: `--start-maximized`, VS Code: `--maximize`. Apps now always fill Xvfb screen after resize instead of restoring saved window geometry from profile |
| 04-26 | Mouse scroll wheel | End-to-end: evdev `REL_WHEEL` → Ring1 `MOUSE_WHEEL` → `SysDisplay_DrainInput` → `Win_ContentEvent(idx, x, y, 3, delta)` → `IPCBroker_SendMouse` → app `xdotool click 4/5`. Event type 3 = scroll, delta in button field (positive=up, negative=down) |
| 04-26 | IPC socket desync recovery | `Socket.RecvMsg` no longer disconnects client on bad length prefix. Drains socket buffer (4KB chunks), returns -1. IPCBroker treats -1 as "bad message, keep connection". Fixes VS Code crash during theme change (extension host restart caused stream desync → "message too large" → window closed) |
| 04-26 | Persistent Xvfb (no restart on resize) | Xvfb launches at full screen resolution (from `UIScale.screen_w`/`screen_h`) instead of window size. On resize: only ShmCanvas destroyed/recreated, `xdotool windowsize` sent to app — zero process restarts, zero state loss. Row-by-row viewport copy (`src_stride = xvfb_w*4`, `dst_stride = canvas_w*4`) with fast-path for full-width case |
| 04-26 | window.created screen dimensions | `IPCBroker_HandleWindowCreate` now includes `screen_w`/`screen_h` from `UIScale` in `window.created` response. IPC apps read these to set Xvfb backing resolution dynamically — no hardcoded 1920x1080 |
| 04-26 | xdotool windowsize on resize | `Chrome_XdotoolResize`/`Vsc_XdotoolResize` send `windowsize --sync 2 W H` to xdotool pipe on resize. App inside Xvfb reflows layout to match new viewport dimensions |
...
**Frame capture:** Xvfb with `-fbdir /tmp` writes its framebuffer to `/tmp/Xvfb_screen0` in xwd format (memory-mapped file). Chrome app mmaps this file read-only at full screen resolution. Each 5ms tick: row-by-row viewport copy from Xvfb mmap (offset 3232) to ShmCanvas. Source stride = `xvfb_w*4`, dest stride = `canvas_w*4`. Fast path when widths match (single MemoryCopy). The 3232-byte offset is the xwd header (160 bytes) + colormap (256 colors × 12 bytes). Pixel format is BGRX (identical to BGRA with alpha=0). No encoding, no pipe I/O, no ffmpeg process.
...
**Privilege drop:** All 3 child forks call `DropPriv()` before `execve`. Stats `/home/bob` for real UID/GID, calls `setgid(gid)` then `setuid(uid)`. Xvfb, Chrome, and xdotool all run as user bob, not root. Required for Electron (refuses root) and correct file ownership.

**Cleanup:** PID file system — PIDs written to `/tmp/chrome_ailang_{xvfb,browser,xdotool}.pid` on launch. `Chrome_CleanupStaleXvfb()` reads PID files, does targeted `kill(pid, SIGKILL)` + `kill(-pid, SIGKILL)` (process group). EPERM prints actionable `sudo kill -9 <pid>`. Removes PID files, fbdir (`rmdir /tmp/chrome_fb`), profile dir (`rm -rf /tmp/chrome_ailang_profile`), Xvfb lock files. `Chrome_StopAll()` uses `Chrome_KillGroup(pid)` for session group kill, removes Singleton locks only (not full profile — preserves tabs/cookies across resize). Never uses broad `pkill`.
...
**Resize:** Persistent — no process restart. Xvfb launches at full screen resolution (read from `UIScale.screen_w`/`screen_h` via `window.created` IPC response). On resize: only ShmCanvas destroyed/recreated at new viewport size, then `xdotool windowsize --sync 2 W H` tells the app to reflow. CopyFrame switches to row-by-row viewport copy (source stride ≠ dest stride). Chrome uses `--start-maximized`, VS Code uses `--maximize` — apps fill the Xvfb screen on initial launch.
...
- VS Code: sandboxed via Xvfb :98 + mmap framebuffer + ShmCanvas, same architecture as Chrome, Ctrl+S/P/N/W/Z/F/G/B/` shortcuts, opens Ailang project dir by default. `--new-window` + `--disable-breakpad` + `--disable-dev-shm-usage` prevent session-joining and crashpad interference. Non-blocking `Vsc_ShellExec` (WNOHANG) prevents close deadlock
