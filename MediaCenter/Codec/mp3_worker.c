/*
 * mp3_worker.c
 * MP3 codec worker — wraps libavcodec MP3 decoder, speaks CodecRing protocol.
 *
 * Usage: mp3_worker <session_id>
 *
 * Reads compressed MP3 packets from:  /dev/shm/ailang_codec_<id>_pkt
 * Writes decoded S16LE PCM frames to: /dev/shm/ailang_codec_<id>_frm
 *
 * Output frames carry:
 *   CRFrameDesc.sample_count  = AVFrame->nb_samples
 ...
 *
 * Build:
 *   gcc -O2 -o mp3_worker mp3_worker.c \
...
int main(int argc, char *argv[]) {
    if (argc < 2) {
        fprintf(stderr, "usage: mp3_worker <session_id>\n");
        return 1;
    }
    ...
    if (!pkt_ring || !frm_ring) {
        fprintf(stderr, "[mp3_worker:%d] ring open failed\n", session_id);
        return 1;
    }

    printf("[mp3_worker:%d] rings open, pkt=%s frm=%s\n",
           session_id, pkt_path, frm_path);

    /* Init libavcodec MP3 decoder */
    const AVCodec *codec = avcodec_find_decoder(AV_CODEC_ID_MP3);
    if (!codec) { fprintf(stderr, "[mp3_worker] no MP3 decoder\n"); return 1; }

    AVCodecContext *ctx = avcodec_alloc_context3(codec);
    if (avcodec_open2(ctx, codec, NULL) < 0) {
        fprintf(stderr, "[mp3_worker] avcodec_open2 failed\n"); return 1;
    }
    ...
    printf("[mp3_worker:%d] decoder ready, entering loop\n", session_id);
    ...
    while (running) {
        ...
        if (hdr.flags & CR_FLAG_EOF) {
            ...
            if (hdr.len > 256 * 1024) {
                fprintf(stderr, "[mp3_worker] oversized packet %u\n", hdr.len);
            ...
            if (ret < 0) {
                fprintf(stderr, "[mp3_worker] send_packet err %d\n", ret);
                continue;
            }
        }
        ...
        while (1) {
            ...
            if (ret < 0) {
                fprintf(stderr, "[mp3_worker] receive_frame err %d\n", ret);
                break;
            }

            /* Init resampler on first frame (we now know sample_rate, channel_layout) */
            if (!swr) {
                ...
                swr_alloc_set_opts2(&swr,
                    ...
                    0, NULL);
                if (swr_init(swr) < 0) {
                    fprintf(stderr, "[mp3_worker] swr_init failed\n");
                    swr_free(&swr);
                    swr = NULL;
                    av_frame_unref(frame);
                    continue;
                }
                printf("[mp3_worker:%d] swr init: %dHz ch=%d → 48000Hz stereo S16LE\n",
