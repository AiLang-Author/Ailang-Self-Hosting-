/*
 * opus_worker.c
 * Opus codec worker — wraps libavcodec Opus decoder, speaks CodecRing protocol.
 *
 * Usage: opus_worker <session_id>
 *
 * Reads compressed Opus packets from:  /dev/shm/ailang_codec_<id>_pkt
 ...
 *
 * Build:
 *   gcc -O2 -o opus_worker opus_worker.c \
...
int main(int argc, char *argv[]) {
    if (argc < 2) {
        fprintf(stderr, "usage: opus_worker <session_id>\n");
        return 1;
    }
    ...
    if (!pkt_ring || !frm_ring) {
        fprintf(stderr, "[opus_worker:%d] ring open failed\n", session_id);
        return 1;
    }

    printf("[opus_worker:%d] rings open, pkt=%s frm=%s\n",
           session_id, pkt_path, frm_path);

    /* Init libavcodec Opus decoder */
    const AVCodec *codec = avcodec_find_decoder(AV_CODEC_ID_OPUS);
    if (!codec) { fprintf(stderr, "[opus_worker] no Opus decoder\n"); return 1; }
    ...
    printf("[opus_worker:%d] waiting for packets\n", session_id);
    ...
    while (running) {
        ...
        if (hdr.flags & CR_FLAG_EOF) {
            ...
            if (hdr.len <= 256 * 1024) {
                ...
                printf("[opus_worker:%d] received extradata %u bytes\n",
                       session_id, hdr.len);
                if (!codec_opened) {
                    if (avcodec_open2(ctx, codec, NULL) < 0) {
                        fprintf(stderr, "[opus_worker] avcodec_open2 failed\n");
                        return 1;
                    }
                    codec_opened = 1;
                    printf("[opus_worker:%d] decoder opened with extradata\n", session_id);
                }
            } else {
                cr_consume(pkt_ring, pkt_buf, hdr.len, total);
            }
            ...
            if (hdr.len > 256 * 1024) {
                fprintf(stderr, "[opus_worker] oversized packet %u\n", hdr.len);
                cr_consume(pkt_ring, pkt_buf, hdr.len, total);
                continue;
            }
            cr_consume(pkt_ring, pkt_buf, hdr.len, total);
            ...
            if (!codec_opened) {
                ...
                codec_opened = 1;
                printf("[opus_worker:%d] decoder opened (no extradata)\n", session_id);
            }
            ...
            if (ret < 0) {
                fprintf(stderr, "[opus_worker] send_packet err %d\n", ret);
                continue;
            }
        }
        ...
        while (1) {
            ...
            if (ret < 0) {
                fprintf(stderr, "[opus_worker] receive_frame err %d\n", ret);
                break;
            }
            ...
            if (!swr) {
                ...
                swr_alloc_set_opts2(&swr,
                    ...
                    0, NULL);
                if (swr_init(swr) < 0) {
                    fprintf(stderr, "[opus_worker] swr_init failed\n");
                    swr_free(&swr);
                    swr = NULL;
                    av_frame_unref(frame);
                    continue;
                }
                printf("[opus_worker:%d] swr init: %dHz ch=%d → 48000Hz stereo S16LE\n",
