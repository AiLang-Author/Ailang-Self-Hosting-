...
_Status: v1 landing · 2026-08-23 · owner: Sean_

## 0. Decisions (2026-08-23)

**One clipboard.** Owned by the display server. Eager copy — the bytes live in
the server, the source app can die, paste still works. Not per-app, not X11
selection-ownership, not “primary vs clipboard.”

**One slot.** History exists internally (`ClipHistory`) but is not the UX.
Multi-slot / clip-history UI waits for demand.

**Text is characters.** v1 stores UTF-8 code units, no attributes. Auckland
syntax coloring and terminal cell colors are *view* state, not document state.
A rich sidecar (`ClipType.RICH`) can sit next to a text fallback later; a
“Paste Special” dialog can then apply attributes. Do not make attributes the
paste-anywhere form — that is how terminals eat RTF.

**Data is the same slot, different type.** `IMAGE` / `BINARY` / `FILE_REF`
reuse the one current payload. Not a second clipboard.
...
- **v1 — system-wide clipboard, unscoped. (landed 2026-08-23)** Promote pool →
  typed slot (`ptr/len/type/seq`); add `clipboard.set/get` (text); native
  textfield path unchanged; terminal Ctrl+Shift+C/V + context-menu copy/paste
  over IPC canvases. Chrome consumes `clipboard.paste` (nested X clip still
  isolated until an xclip bridge). _This is the first buildable slice._
